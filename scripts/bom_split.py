...
-- those attributes live on the board itself. The hand-solder set is itself derived:
...
NO_PART_FAMILIES = {
    ...
    # NT1 is MouseBiteLabs' single-point-ground net tie -- the one place the audio
    ...
    # through to the hand-solder list with the "no recorded reason -- add one" fallback. This
    # is that reason.
...
def classify(fp):
    ...
    for key, why in NO_PART_FAMILIES.items():
        if key.lower() in fam.lower():
            return "none", why
    if fp.bom_excluded:
        # The reason lives in the generator beside the flag it explains, so the buy
        # document and the board cannot give different accounts of the same decision.
        why = (build_board.SALVAGE_ONLY.get(fp.ref)
               ...
               or ("ClockxControl mezzanine -- its plated holes are filled with solder "
                   ...
                   if fp.ref == "MOD1" else
                   "BOM-excluded on the board with no recorded reason -- add one"))
        return "hand", why
    return "assembly", ""
...
def build(boards=1, board=None):
    ...
    for fp in kisexp.footprints(board):
        ...
        row = rows[kind].setdefault(key, {
            ...
            # footprint" -- and an 'eco' is why the part was chosen. Emitting only the
            # first present once displaced that warning out of the CSV PCBWay reads.
...
def _handbuy_md(hand, boards):
    L = [f"# Hand-buy list — {STEM}",
         ...
         "list by changing the design — see [DESIGN-DECISIONS.md]"
