...
There are **two** zips, and uploading the wrong one is what got the first submission bounced.

| File | Where it goes | What is in it |
|---|---|---|
| [`fab/agbm-02-cxc-gerbers.zip`](fab/agbm-02-cxc-gerbers.zip) | **PCBWay's "PCB file" box** | 13 files, **flat**, fabrication data only |
| [`fab/agbm-02-cxc-pcbway.zip`](fab/agbm-02-cxc-pcbway.zip) | the record; assembly documents come out of it | 20 files in folders, plus the BOM, CPL and `ORDER.txt` |

**Why two.** The first upload was the full package, and PCBWay's file review came back with
*"there is no drill file included in your design."* Both drill files were present and both are
valid Excellon — `M48`, metric, `FMAT,2`, 9 tools / 640 hits / 10 slots on the PTH file and 1 tool
/ 2 hits on the NPTH — but they sat one directory down in `drill/`. **Intake reads the archive
flat.** A drill file in a folder is a drill file nobody sees. So the upload zip has no folders.

Two things are also left out of the upload on purpose:

- **The drill maps.** They are human-readable plots carrying a `.gbr` extension, and a `.gbr` that
  is not a real layer is what makes a viewer report a layer count nobody can explain.
- **The `.gbrjob`.** KiCad writes `"BoardThickness": 1.2` and `"Finish": "None"` into it. This
  board is ordered at **1.0 mm with ENIG**. Its layer identification is redundant — the Protel
  extensions already say which file is which — so the only thing it would add is two values that
  contradict the order form, in a file a fab's intake may read in preference to a human. Check [21]
  fails if any file in the upload ever declares a thickness that disagrees with the spec again.

The record zip is generated by [`scripts/fab_package.py`](../scripts/fab_package.py) and contains:
