...
bare, it is copper and mask, and the module already sits over 25 of MouseBiteLabs' own vias.
...
DNP. The current hand set is `P1`, `P3`, `P4`, `SP1`, `VR2` and `U1`.
...
`TSOP-I-48_18.4x12mm_P0.5mm` land — the *outer* pattern, 20.950 mm lead-tip span. Only that
pattern is pasted; pasting both would reflow solder under the body of the part that is
actually fitted, across pads 0.5 mm apart on different nets.
...
never arm. `Q2`, `Q5` and `Q7` keep the NDC7002N. `PTC1` and `F1` also carry corrected
descriptions — the review's derating numbers were optimistic for a Low Rho part.
...
dropping that assumption took the search from 492 legal sites to 3,659 front and 6,327 back.
None of the six is legal on the other side.
...
Tracks are compared by **coverage**, not segment by segment: KiCad 10 merges collinear
tracks, so the same copper is stored as 3,554 segments in one file and fewer in the other. The
comparison groups by layer, net, width and the infinite line each segment lies on, then merges
intervals — invariant under merging, and still catches anything that moved.
